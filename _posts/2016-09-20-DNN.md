...
While ordinary recurrent networks are good for finite-delays, when the sequence of recurrence goes back in time by a significant order, the gradients can become intractable. For example, consider a network node whose output unit \\(k\\) at time \\(t\\) is \\(y\_k(t)\\) so that the error computed for node \\(k\\) is 
