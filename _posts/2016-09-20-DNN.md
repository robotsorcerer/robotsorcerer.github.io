...
where \\(f(\cdot) \\) is a suitably chosen nonlinear, bounded continuous function. If the system under consideration is a continuous-time system, the dynamical system in the feedback path has a diagonal transfer matrix with elements \\(1/(s + \alpha) \\) along the diagonal <a href="#fn4" class="footnoteRef" id="fnref4"><sup>4</sup></a>. \\(\alpha \\) is an unknown parameter to that parameterizes the lagged outputs; the system is then represented by the equation
