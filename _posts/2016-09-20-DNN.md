...
The chief idea in system identification experiments is to charcterize the behavior of a plant by an operator \\( \mathbb{F} \\) that resolves the dynamics of a plant based on input-output datapair \\( \{u\_i(k), y\_i(k)\} \\)  for all \\(i = 1, 2, \cdots , N \\). Previously, we used an LTI model generated from the <i>prediction error model</i> in characterizing system and found it insufficienty to approximate the system. So here we propose Hammerstein block-structured models, with the nonlinear element approximated by recurrent neural networks and the linear dynamical subsystem approximated by a multilayer network. Together, this can be summarized by the figure below

<div class="fig figcenter fighighlight"> 
<img src="/imgs/NNDyn/ham.png" padding-right="20cm"/>
<div class="figcaption" align="left">Fig 2: The Hammerstein block-structured model.</div>
</div>
