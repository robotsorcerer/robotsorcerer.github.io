...
> This post accompanies my recent work, *Visuomotor Diffusion Policies via Inverse
> PDEs* ([RoboDiff]({{ site.baseurl }}/downloads/Papers/robodiff.pdf)). It distills two
> companion notes — a first-principles derivation of the **backward Kolmogorov equation
> (BKE)** in infinite dimensions, and a hands-on **PushT** exposition notebook — into one
> narrative. The claim in a sentence: *for long-horizon planning, don't diffuse points —
> diffuse functions.*

## Why point-based diffusion policies drift

A modern diffusion policy samples an action **chunk** $$a \in \mathbb{R}^{T_p \times d_a}$$ by
running a denoiser backward from Gaussian noise. That works beautifully at short horizons.
But the moment you stack the horizon $$T_p$$ high — the regime you actually want for
long-horizon manipulation and planning — the geometry turns against you. You are denoising
in $$\mathbb{R}^{N}$$ with $$N = T_p \cdot d_a$$, and the standard DDPM convergence bound reads

$$\|\mu_\theta - \mu_{\text{data}}\|_{\text{TV}} = O\!\left(\sqrt{N}\,\sqrt{\text{loss}}\right).$$

The $$\sqrt{N}$$ is the villain. Refine the temporal grid and the bound *degrades*. White
noise $$\mathcal{N}(0, I)$$ treats every timestep as an independent coordinate, so the sampler
has no notion that a trajectory should be *smooth in time*. The result is the familiar
long-horizon drift.

The fix is not a better network. It is a better **space**.

## The move: diffuse in a Hilbert space

Instead of an action chunk being a vector, treat it as an element of the function space

$$\mathcal{H} = L^2\!\left([0,T],\, \mathbb{R}^{d_a}\right),$$

the square-integrable action *trajectories*. This is infinite-dimensional from the start —
there is no grid to explode. The whole machinery of diffusion (a forward noising process, a
score, a reverse SDE) lifts to $$\mathcal{H}$$, but three objects have to be built with care:
