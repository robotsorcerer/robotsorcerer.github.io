...
To illustrate the way the reconstruction error can make the life of a control designer really miserable,  I shall be borrowing the example from Polycarpou's IEEE TAC 1996 paper on ["Stable Adaptive Neural Control Scheme for Nonlinear Systems"][polycarpou].
...
The network reconstruction error is very crucial in representing the _minimum possible deviation_ from the unknown function \\(\phi\\) and the I/O of the function approximator. Genrally, by the [universal approximation theorem for neural networks](approx-net), one can make \\(\delta\\) arbitrarily small on an compact set by making the number of parameters (or weights) i.e. \\(n\\) really large.
...
[approx-net]: Funahashi, Ken-Ichi (1989). On the approximate realization of continuous mappings by neural networks Neural Networks. Elsevier, 1989.
